...
namespace HotelManager
{
    public partial class fUseService : Form
    {
        ...
        public void Pay(string idBill, int discount)
        {
        ...
        public void LoadListService(string idServiceType)
        {
        ...
        {
            string idRoomTypeName = RoomTypeDAO.Instance.GetRoomTypeByIdRoom(room.Id).Id;
        ...
        private void Button_Click(object sender, EventArgs e)
        {
            ...
            button.color = System.Drawing.Color.SteelBlue;
            button.colorActive = System.Drawing.Color.SteelBlue;
            ...
            {
                string idReceiveRoom = ReceiveRoomDAO.Instance.GetIdReceiveRoomFromIdRoom(room.Id);
                string idbill = GetAutomaticID();
                //string idbill = BillDAO.Instance.GetIdBillFromIdRoom(room.Id);
                InsertBill(idbill, idReceiveRoom, staffSetUp);
            }
        ...
        //thêm bill
        public void AddBill(string idRoom, string idService,int count)
        {
            if(IsExistsBill(idRoom))
            {
                ...
                if(!IsExistsBillDetails(idRoom,idService))
                {
                    ...
                    if (count > 0)
                    {
                        string idBill = BillDAO.Instance.GetIdBillFromIdRoom(idRoom);
            ...
            else
            {
                ...
                if (count > 0)
                {
                    string idReceiveRoom = ReceiveRoomDAO.Instance.GetIdReceiveRoomFromIdRoom(idRoom);
                    string idbill = GetAutomaticID();
                    InsertBill(idbill,idReceiveRoom, staffSetUp);
                    string idBill = BillDAO.Instance.GetIdBillMax();
                    InsertBillDetails(idBill, idService, count);
                }
                else
                    MetroFramework.MetroMessageBox.Show(this, "Số lượng không hợp lệ.\nVui lòng nhập lại.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public void ShowBillRoom(string idRoom)
        {
            CultureInfo cultureInfo = new CultureInfo("vi-vn");
            listViewBillRoom.Items.Clear();
            if (IsExistsBill(idRoom))
        ...
        //thay đổi combobox loại dịch vụ
        ...
        {
            return GetIDAutomaticDAO.Instance.Actomatic_ID("Bill", "ID");
        }
        //kiểm tra bill tồn tại hay không
        public bool IsExistsBill(string idRoom)
        {
            return BillDAO.Instance.IsExistsBill(idRoom);
        }
        //kiểm tra chi tiết bill có tồn tại hay không
        public bool IsExistsBillDetails(string idRoom, string idService)
        {
        ...
        {
            return BillDAO.Instance.InsertBill(id, idReceiveRoom, staffSetUp);
        }
        //insert detail bill
        public bool InsertBillDetails(string idBill, string idService, int count)
        {
            return BillDetailsDAO.Instance.InsertBillDetails(idBill, idService, count);
        }
        //cập nhật bill
        public bool UpdateBillDetails(string idBill, string idService, int _count)
        {
